...
interface NewsWithProblematicImage {
  id: string;
  title: string;
  imageUrl: string;
  createdAt: Date;
 ...
 */
function generateApiUrl(newsId: string): string {
 ...
 */
async function findProblematicNews(): Promise<NewsWithProblematicImage[]> {
  console.log('🔍 Buscando noticias con URLs problemáticas...');
  
  // Query para encontrar noticias con rutas del filesystem
  const problematicNews = await prisma.$queryRaw<NewsWithProblematicImage[]>`
    SELECT 
      "id",
      "title", 
      "imageUrl",
 ...
 */
function analyzeProblematicNews(news: NewsWithProblematicImage[]): void {
 ...
 */
async function migrateNewsImage(news: NewsWithProblematicImage): Promise<boolean> {
  try {
 ...
 */
async function processMigration(news: NewsWithProblematicImage[]): Promise<MigrationResult> {
  console.log('\n🔄 INICIANDO MIGRACIÓN:');
  console.log('='.repeat(60));
...
async function validateMigration(): Promise<void> {
  ...
  const migratedNews = await prisma.$queryRaw<Array<{id: string, imageUrl: string}>>`
    SELECT "id", "imageUrl" 
    FROM "news" 
